...
Gemini may only use subagents when the user explicitly asks for delegation or parallel agent work. Do not force delegation when the runtime instructions do not allow it.
...
At the end of a meaningful change, update the existing project memory file if the repository already uses one, such as `GEMINI.md`, `AGENT.md`, or another established top-level guide.
